...
static void app(void) {
  ...
  while (1) {
    ...
    if (FD_ISSET(STDIN_FILENO, &rdfs)) {
      ...
      // send usage help to the client
      send_usage_to_client(c.sock);
    } else {
      bool action = false;
      int i = 0;
      for (i = 0; i < actual; i++) {
        ...
        if (FD_ISSET(clients[i].sock, &rdfs)) {
          ...
          if ((strstr(buffer, "Private to ") - buffer == 0) && (!action)) {
            ...
            strncpy(receiverName, receiverNamebuff,
                    (strstr(receiverNamebuff, " : ") - receiverNamebuff));
            int length = strlen(receiverName) + 3;
            char *message = &receiverNamebuff[length];
            for (int j = 0; j < actual; j++) {
              if ((strcmp(clients[j].name, receiverName) == 0) && (!action)) {
                send_message_to_a_client(clients, client, clients[j], actual,
                                         message, 0);
              }
            }
            action = true;
          }
          if ((strstr(buffer, "See Clients") - buffer == 0) && (!action)) {
            for (int j = 0; j < actual; j++) {
              puts(clients[j].name);
            }
            action = true;
          }
          if ((strstr(buffer, "See Groups") - buffer == 0) && (!action)) {
            for (int j = 0; j < actualGroupe; j++) {
              printf("%s (%d)", groupes[j].name, groupes[j].actualMembre);
              for (int k = 0; k < groupes[j].actualMembre; k++) {
                printf("\nMember Num %d : %s", k, groupes[j].membres[k].name);
              }
              printf("\n");
            }
            action = true;
          ...
          if ((strstr(buffer, "To ") - buffer == 0) && (!action)) {
            ...
            strncpy(groupName, groupNamebuff,
                    (strstr(groupNamebuff, " : ") - groupNamebuff));
              ...
              if (strcmp(groupes[j].name, groupName) == 0) {
                for (int k = 0; k < groupes[j].actualMembre; k++) {
                  if (strcmp(groupes[j].membres[k].name, client.name) == 0) {
                    send_message_to_a_group(groupes[j].membres, client,
                                            groupes[j].actualMembre, message,
                                            groupName, 0);
                  }
                }
              }
...
static void send_message_to_all_clients(Client *clients, Client sender,
  ...
  for (i = 0; i < actual; i++) {
    // puts(strncat(strncat(sender.name, " : ", sizeof sender.name -
    // strlen(sender.name) - 1), buffer, sizeof sender.name -
    // strlen(sender.name) + 3));
...
static void send_message_to_a_client(Client *clients, Client sender,
                                     Client receiver, int actual,
  ...
  for (i = 0; i < actual; i++) {
    ...
    if (receiver.sock == clients[i].sock) {
      ...
      strncat(message, buffer, sizeof message - strlen(message) - 1);
      strncat(serverMessage, "For ", BUF_SIZE - 1);
      strncat(serverMessage, receiver.name, BUF_SIZE - 1);
      strncat(serverMessage, " from ", BUF_SIZE - 1);
...
static void create_groupe(Client *clients, const char *buffer, int nbMembre) {
  ...
  message_acceuil[0] = 0;
  strcpy(message_acceuil, "New Group");
  strcat(message_acceuil, groupe.name);
  strcat(message_acceuil, " composed of");
...
static void add_groupe_member(Client client, const char *buffer) {
  for (int j = 0; j < actualGroupe; j++) {
    if (strcmp(groupes[j].name, buffer) == 0) {
      ...
      strcat(message_acceuil, " joined the group ");
...
static void delete_groupe_member(Client client, const char *buffer) {
  for (int j = 0; j < actualGroupe; j++) {
    if (strcmp(groupes[j].name, buffer) == 0) {
      ...
      message_acceuil[0] = 0;
      strcpy(message_acceuil, client.name);
      strcat(message_acceuil, " left the group ");
      strcat(message_acceuil, groupes[j].name);
      strcat(message_acceuil, " composed of");
      for (int i = 0; i < groupes[j].actualMembre; i++) {
        strcat(message_acceuil, " ");
...
static void send_usage_to_client(SOCKET sock) {
  ...
  for (size_t i = 0; i < count; ++i) {
    write_client(sock, help_lines[i]);
  }
}
